...
@class HSegmentItem;
@protocol HSegmentItemDelegate

- (void)ItemStateChanged:(HSegmentItem *)item index:(NSInteger)index isSelected:(BOOL)isSelected;
@end

#pragma mark - HSegmentItem
@interface HSegmentItem : UIView
...
- (instancetype)initWithFrame:(CGRect)frame items:(NSArray *)items;
{
    ...
    if (self) {
        ...
        for (NSInteger i = 0; i < count; i++) {
            HSegmentItem *item = [[HSegmentItem alloc] initWithFrame:CGRectZero
                                                                 index:i
                                                                 title:items[i]
...
- (void)layoutSubviews{
    ...
    //configure items
    [self.items enumerateObjectsUsingBlock:^(HSegmentItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)setTintColor:(UIColor *)tintColor{
    ...
    if (_tintColor != tintColor) {
        ...
        for (NSInteger i = 0; i<self.items.count; i++) {
            HSegmentItem *item = self.items[i];
...
#pragma mark - HSegmentItemDelegate
- (void)ItemStateChanged:(HSegmentItem *)currentItem index:(NSInteger)index isSelected:(BOOL)isSelected{
