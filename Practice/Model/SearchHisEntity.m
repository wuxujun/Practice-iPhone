...
-(void)setValue:(id)value forKey:(NSString *)key
{
    ...
    }else if([key isEqualToString:@"hisKeyword"]){
        self.hisKeyword=value;
...
+(NSArray*)fields
{
    return [NSArray arrayWithObjects:@"id",@"hisType",@"hisKeyword",@"state",@"addtime", nil];
}
...
+(void)generateInsertSql:(NSDictionary *)info completion:(SqlBlock)completion
{
    ...
    for (int i=0; i<values.count; i++) {
        ...
        NSString *key = keys[i];
        NSArray *integerKeyArray = @[@"id",@"hisType",@"state",@"addtime"];
...
+(void)generateUpdateSql:(NSDictionary *)info completion:(SqlBlock)completion
{
    ...
    NSString *sql = [NSString stringWithFormat:@"UPDATE t_search_his set %@ WHERE id=%@", [kvPairs componentsJoinedByString:@", " ], mID];
